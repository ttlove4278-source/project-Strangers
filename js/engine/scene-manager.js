const SceneManager = {
    ...
    async switchTo(screenId, transition = 'crt') {
        ...
        if (transition === 'crt' && this.currentScreen) await Transitions.crtOff();
        else if (transition === 'fade' && this.currentScreen) await Transitions.fadeOut(400);
        Object.values(this.screens).forEach(s => s.classList.remove('active'));
        target.classList.add('active');
        this.currentScreen = target;
        Transitions.sceneTransition(screenId);
        await Effects.wait(100);
        if (transition === 'crt') await Transitions.crtOn();
        this.transitioning = false;
    },
    getCurrentId() { return this.currentScreen ? this.currentScreen.id : null; }
};
