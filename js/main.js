const Game = {
    ...
    async onAwakeningComplete() {
        ...
        await Effects.showFullscreenText('蝉在叫。太阳不会落。<br>你开始推石头。', 2500);
        await ExplorationEngine.enter('teibow');
    },

    showDeathMemories() {
        if (!this.state.deathMemories?.length) { Effects.notify('死亡记忆为空', 2000); return; }
    ...
    async handleDeath(memory = '未记录') {
        this.state = SaveManager.deathReset(this.state, {
            scene: this.state.currentScene, location: this.state.currentLocation,
